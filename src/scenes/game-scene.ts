...
import { addAnyPercentLevelTime, saveBestAnyPercentTimeIfBetter, stopAnyPercentRun } from '../utils/any-percent'
...
export default class GameScene extends Phaser.Scene {
  ...
  private anyPercentLevelWallStartTime = 0
  ...
  startTimer() {
    ...
    this.anyPercentLevelWallStartTime = Date.now()
  ...
  teleport() {
    ...
    const previousBestTimeBeforeUpdate = this.currentLevel ? getLevelInfo(this.currentLevel)?.time || Infinity : Infinity
    const hasExistingGhostBeforeUpdate = this.currentLevel ? !!getGhostRun(this.currentLevel) : false
    ...
    if (this.isSpeedrunMode && this.currentLevel && !this.startedFromCheckpoint) {
      ...
      // On garde le fantôme seulement si c'est un nouveau record (ou s'il n'y en avait pas encore)
      if ((newTime < previousBestTimeBeforeUpdate || !hasExistingGhostBeforeUpdate) && this.recordedGhostFrames.length > 1) {
      ...
      const anyPercentLevelTime = Date.now() - this.anyPercentLevelWallStartTime
      const anyPercentState = addAnyPercentLevelTime(world, this.currentLevel, anyPercentLevelTime)
      if (anyPercentState && isLastLevelOfWorld) {
        saveBestAnyPercentTimeIfBetter(world, anyPercentState.totalTime)
        stopAnyPercentRun()
      }
    }
